...
for (setting in 1:total_setting){
  ...
  write.table(mse, paste('stimulate/results/stimulation_2/multi-tsls/multi-tsls.mse.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(coef, paste('stimulate/results/stimulation_2/multi-tsls/multi-tsls.coef.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
