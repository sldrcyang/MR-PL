...
run_each = function(setting, cutoff, c){
  ...
  for (idx in 1:total_replication){
    ...
    ###2).causal effect
    tmp.coef = data.frame(setting=setting, replicate=idx, image_name=1:20, coef=full_coef_val, if_include=if_include)#拟合值
  ...
  write.table(coef.2, paste('stimulate/results/winners curse/stimulation_4/correct for Nimage/coef.cutoff_',cutoff,'.c_',c,'.setting_', 
                           setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
