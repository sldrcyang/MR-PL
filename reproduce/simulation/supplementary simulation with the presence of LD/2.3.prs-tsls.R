...
my_utsls = function(outcome_matrix, exposure_matrix, g_matrix){
...
run_each_setting = function(setting){
  ...
  for (idx in 1:total_replication){
    ...
    for (n_col in 1:ncol(exposure_matrix)){
      ...
      prs = apply(g_matrix, 1, mean)
      prs = scale(prs, center = TRUE, scale = TRUE)
      g_matrix = as.vector(prs)
      
      tsls = my_utsls(outcome_matrix, exposure_matrix[,n_col], g_matrix)
  ...
  write.table(mse, paste('stimulate/results/stimulation_4/prs+uni-tsls/prs5e-08+uni-tsls.mse.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(coef, paste('stimulate/results/stimulation_4/prs+uni-tsls/prs5e-08+uni-tsls.coef.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
