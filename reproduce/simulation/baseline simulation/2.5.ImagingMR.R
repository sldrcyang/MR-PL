...
save_imagingMR_LAS_results <- function(setting, dataX, dataY, dataM, p, q, alpha){
  ######################### 2. MR-single IV #########################
  ...
  for (h in 1:p) {  ## MR using single SNP, h:snp??index
...
run_each_setting = function(setting){
  ...
  for (idx in 1:total_replication){
    ...
    LAS_results = save_imagingMR_LAS_results(setting, dataX, dataY, dataM, p=ncol(dataX), q=ncol(dataM), cutoff)
  ...
  write.table(mse, paste('stimulate/results/stimulation_2/las/las.mse.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(coef, paste('stimulate/results/stimulation_2/las/las.coef.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
