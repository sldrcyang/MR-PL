...
run_each_setting = function(setting){
  ...
  for (idx in 1:total_replication){
    ...
    ###2). causal effect
    tmp.coef = data.frame(setting=setting, replicate = rep(idx, 20), image_name=1:20, coef=reg.coef) #????ֵ
    tmp.coef$true_beta = beta_xToy 
  ...
  write.table(mse.1, paste('stimulate/results/stimulation_2/pls+ridge/pls+ridge.mse.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(coef.1, paste('stimulate/results/stimulation_2/pls+ridge/pls+ridge.coef.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  
  write.table(mse.2, paste('stimulate/results/stimulation_2/pls+lasso/pls+lasso.mse.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(coef.2, paste('stimulate/results/stimulation_2/pls+lasso/pls+lasso.coef.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  
  write.table(mse.3, paste('stimulate/results/stimulation_2/pls+elasticnet/pls+elasticnet.mse.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(coef.3, paste('stimulate/results/stimulation_2/pls+elasticnet/pls+elasticnet.coef.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
