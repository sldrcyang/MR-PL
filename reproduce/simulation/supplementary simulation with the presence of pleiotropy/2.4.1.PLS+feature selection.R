...
u_tao = 0.2; sd_tao = 0.05    #u_tao_set = c(0,0.05,0.2); sd_tao_set = c(0.05,0.1)
#dir.create(paste('stimulate/results/stimulation_8/pls+ridge/u_tao',u_tao,'sd_tao',sd_tao, sep = ''),  showWarnings = F)
...
run_each_setting = function(setting){
  ...
  for (idx in 1:total_replication){
    ...
    tmp.sargan.1 = data.frame(setting=setting, replicate=idx, sargan_p = sargan.p)
    sargan.1 = rbind(sargan.1, tmp.sargan.1)
    ...
    sargan.stat = nrow(g_matrix) * R2
    df = ncol(g_matrix) - ncol(pls.pred)
    sargan.p2 = 1 - pchisq(sargan.stat, df = df)
    #----------------------------------------------------------------------
    ...
    ###3.p-value of sargan test
    tmp.sargan.3 = data.frame(setting=setting, replicate=idx, sargan_p = sargan.p)
    sargan.3 = rbind(sargan.3, tmp.sargan.3)
    ...
    ##Sargan test ##----------------------------------
    outcome_predicted = predict(reg.mod, type = 'fit', newx = pls.pred) [["fit"]]  [,beststep]
    residual = outcome_matrix - outcome_predicted
    ...
    ###3.p-value of sargan test
    tmp.sargan.4 = data.frame(setting=setting, replicate=idx,sargan_p_adj = sargan.p)
    sargan.4 = rbind(sargan.4, tmp.sargan.4)
    
    cat(idx, '\n')
  }
  
  
  write.table(mse.1, paste('stimulate/results/stimulation_8/pls+ridge/u_tao',u_tao,"sd_tao",sd_tao,'/pls+ridge.mse.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(coef.1, paste('stimulate/results/stimulation_8/pls+ridge/u_tao',u_tao,"sd_tao",sd_tao,'/pls+ridge.coef.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(sargan.1, paste('stimulate/results/stimulation_8/pls+ridge/u_tao',u_tao,"sd_tao",sd_tao,'/pls+ridge.sargan.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  
  write.table(mse.2, paste('stimulate/results/stimulation_8/pls+lasso/u_tao',u_tao,"sd_tao",sd_tao,'/pls+lasso.mse.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(coef.2, paste('stimulate/results/stimulation_8/pls+lasso/u_tao',u_tao,"sd_tao",sd_tao,'/pls+lasso.coef.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(sargan.2, paste('stimulate/results/stimulation_8/pls+lasso/u_tao',u_tao,"sd_tao",sd_tao,'/pls+lasso.sargan.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  
  write.table(mse.3, paste('stimulate/results/stimulation_8/pls+elasticnet/u_tao',u_tao,"sd_tao",sd_tao,'/pls+elasticnet.mse.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(coef.3, paste('stimulate/results/stimulation_8/pls+elasticnet/u_tao',u_tao,"sd_tao",sd_tao,'/pls+elasticnet.coef.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  write.table(sargan.3, paste('stimulate/results/stimulation_8/pls+elasticnet/u_tao',u_tao,"sd_tao",sd_tao,'/pls+elasticnet.sargan.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
  
  write.table(mse.4, paste('stimulate/results/stimulation_8/pls+lars/u_tao',u_tao,"sd_tao",sd_tao,'/pls+lars.mse.setting_',setting,'.txt', sep=''), sep="\t", row.names = F, quote = F)
