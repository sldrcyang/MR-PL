...
#########################################simulate genotype matrix (the same as baseline simulation)####################################
...
#  g_matrix [,i] = one_snp     # n*p matrix 最终生成的基因型
#}
...
stimulation = function(setting, hg2.fomul, beta_ux_vector.fomul, beta_xToy.fomul, sd_error_x, sd_error_y,u_tao,sd_tao){
  for (idx in 1:100) {
    ...
    ##########################simulate pleiotropy effect 5000*1######################################
    tao_g = rnorm(n_snp, u_tao, sd = sd_tao) 
    
    
    ##################################simulate outcome 10000 *1 ######################################
    outcome_matrix = matrix(NA, n_sample, 1)
    outcome_matrix[,1] = g_matrix %*% tao_g + exposure_matrix %*% beta_xToy + u_matrix %*% beta_uy_vector + error_y_matrix[,1] 
    ...
    #only save the outcome_matrix, others are the same as baseline simulation
    save(outcome_matrix, file=paste("stimulate/exposure-outcome/stimulation_8/u_tao",u_tao,"sd_tao",sd_tao,"/", setting,'/', idx,".exposure-outcome-iv", ".RData", sep=""))
  }
}
...
for (hg2 in hg2_list){
  for (beta_ux_vector in beta_ux_vector_list){
    for (beta_xToy in beta_xToy_list){
      ...
      dir.create(paste('stimulate/exposure-outcome/stimulation_8/u_tao',u_tao,"sd_tao",sd_tao, sep = ''),  showWarnings = F)
      dir.create(paste('stimulate/exposure-outcome/stimulation_8/u_tao',u_tao,"sd_tao",sd_tao,"/", setting, sep = ''),  showWarnings = F)
...
result = foreach(id_row = 1:nrow(setting.summary), .combine = 'rbind') %dopar% run_each_setting(id_row)  #node03
stopCluster(cl)
